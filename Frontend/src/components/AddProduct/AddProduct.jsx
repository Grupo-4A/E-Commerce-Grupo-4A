...
import { fetchCategories, fetchBrands } from '../../services/dataService'; // Cambiado de categoryService y brandService
...
const AddProduct = () => {
  ...
  const fetchInitialData = async () => {
    try {
      // Usando las funciones renombradas de dataService
      const [categoriesData, brandsData] = await Promise.all([
        fetchCategories(), // Cambiado de getAllCategories()
        fetchBrands() // Cambiado de getAllBrands()
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const productData = {
        ...
        ...(formData.ramValue && { ramValue: formData.ramValue }),
        ...(formData.diskSpaceValue && { diskSpaceValue: formData.diskSpaceValue }),
        ...(formData.compatibilityIds.length > 0 && { compatibilityIds: formData.compatibilityIds }),
        ...(formData.licenseIds.length > 0 && { licenseIds: formData.licenseIds })
      };
      ...
      setErrors({ general: 'Error al crear el producto: ' + error.message });
    } finally {
      setLoading(false);
      setUploadingImage(false);
    }
            ...
            >
              {loading || uploadingImage ? (
                <>
                  ...
                  {uploadingImage ? 'Subiendo imagen...' : 'Creando producto...'}
                </>
              ) : (
                <>
